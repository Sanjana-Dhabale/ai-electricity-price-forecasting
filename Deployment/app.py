...
import os  # Add this import at the very top of your file if it's not there

# --- 1. LOAD RESOURCES & DATA (FIXED VERSION) ---
@st.cache_resource
def load_resources():
    
    # 1. Get the exact path where app.py is located
    current_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 2. Construct the full paths to your files
    model_path = os.path.join(current_dir, 'model_price_forecast.joblib')
    scaler_path = os.path.join(current_dir, 'scaler.joblib')
    csv_path = os.path.join(current_dir, 'processed_dataset.csv')
    
    # 3. Load them using these full paths
